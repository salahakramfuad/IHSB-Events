...
function LoginForm() {
  ...
  async function finishSignIn(userCred: { user: { getIdToken: (forceRefresh?: boolean) => Promise<string>; uid: string; email: string | null } }) {
    ...
    let target = redirect || (isAdmin ? '/admin' : '/dashboard')
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const userCred = await signInWithPopup(authInstance, new GoogleAuthProvider())
      await finishSignIn(userCred)
    } catch (err: unknown) {
      const code = err && typeof err === 'object' && 'code' in err ? String((err as { code: string }).code) : ''
      if (code === 'auth/popup-closed-by-user' || code === 'auth/cancelled-popup-request') {
        setError('')
      } else {
        setError(err && typeof err === 'object' && 'message' in err ? String((err as { message: string }).message) : 'Google sign-in failed.')
      }
    } finally {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 px-4 py-12">
      <div className="w-full max-w-md">
        <div className="rounded-2xl border border-slate-200/80 bg-white p-8 shadow-lg">
          ...
          <h1 className="text-2xl font-bold text-slate-900 text-center mb-2">Student sign in</h1>
          <p className="text-center text-slate-500 text-sm mb-6">
            Sign in to register for events and view your dashboard.
          </p>
          ...
          <button
            ...
            className="w-full inline-flex items-center justify-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:opacity-60"
          >
