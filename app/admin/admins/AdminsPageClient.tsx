...
export default function AdminsPageClient({ admins: initialAdmins, forbidden }: AdminsPageClientProps) {
  const router = useRouter()
  const [list, setList] = useState(initialAdmins)
  const [error, setError] = useState(forbidden ? 'You do not have permission to view admins.' : '')
  ...
  useEffect(() => {
    setList(initialAdmins)
  }, [initialAdmins])

  useEffect(() => {
    if (forbidden) setError('You do not have permission to view admins.')
  }, [forbidden])

  const refreshAdmins = () => router.refresh()
