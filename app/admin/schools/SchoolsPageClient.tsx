...
import type { SchoolWithStats } from '@/app/admin/actions'

type School = SchoolWithStats
...
export default function SchoolsPageClient({
  ...
  const [list, setList] = useState(initialSchools)
  ...
  const handleEventFilterChange = (eventId: string) => {
    ...
    const url = eventId ? `/admin/schools?eventId=${eventId}` : '/admin/schools'
    router.replace(url)
  }
